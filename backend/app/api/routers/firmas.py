...
from app.schemas.documents import DocumentsIn, DocumentsOut
...
@router.post("/{pyme_id}/firmar_documento", status_code=status.HTTP_201_CREATED, response_model=DocumentsOut)
async def firmar_documento_para_pyme(
    ...
    """Sign a document for a Pyme. Minimal, demo-friendly implementation."""
    # For demo/testing we generate a temporary key pair and sign the content
    private_pem, public_pem = generar_par_claves()
    signature_b64 = firmar_documento(documento.contenido, private_pem)

    # Persist minimal Document using SQLAlchemy core via session
    new_doc = Document(pyme_id=pyme_id, contenido=documento.contenido, firma_digital=signature_b64, fecha_firma=datetime.utcnow())
    db.add(new_doc)
    await db.commit()
    await db.refresh(new_doc)

    # Return document plus public key (for tests only)
    out = DocumentsOut.model_validate(new_doc) if hasattr(DocumentsOut, 'model_validate') else DocumentsOut.from_orm(new_doc)
    # attach public key for test verification (not for production)
    try:
        out_dict = out.model_dump() if hasattr(out, 'model_dump') else out.dict()
    except Exception:
        out_dict = out if isinstance(out, dict) else dict(out)
    out_dict["public_key_pem"] = public_pem.decode('utf-8')
    return out_dict


@router.post("/{pyme_id}/verificar_documento/{documento_id}")
async def verificar_documento_de_pyme(
    pyme_id: uuid.UUID = Path(...),
    ...
    # Load document
    doc = await db.get(Document, documento_id)
    if doc is None or str(doc.pyme_id) != str(pyme_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Document not found")

    valido = verificar_firma(doc.contenido, doc.firma_digital, public_key_pem.encode('utf-8'))
