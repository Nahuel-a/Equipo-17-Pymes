...
from app.schemas.documents import ApplicationDocumentBase, ApplicationDocumentSchema
...
@app.post("/application_documents/", response_model=ApplicationDocumentSchema)
async def create_document(
    document: ApplicationDocumentBase, db: AsyncSession = Depends(get_session)):
    db_document = await ApplicationDocumentCrud(db).create(document)
    return db_document
    """
...
@app.get("/application_documents/{doc_id}", response_model=ApplicationDocumentSchema)
...
@app.get("/application_documents/", response_model= [ApplicationDocumentSchema])
...
@app.put("/application_documents/{doc_id}", response_model=ApplicationDocumentSchema)
async def update_application_document(
    ...
    """
    db_document = db.query(ApplicationDocumentBase).filter(ApplicationDocumentBase.id == doc_id).first()
...
@app.delete("/application_documents/{doc_id}", status_code= 'status.HTTP_204_NO_CONTENT')
